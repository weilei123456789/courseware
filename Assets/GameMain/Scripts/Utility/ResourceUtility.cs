...
namespace Penny
{
    public static class ResourceUtility
    {
        ...
        public static void LoadUISprite(string fileName, Sprite sprite)
        {
            GameEntry.Resource.LoadAsset(AssetUtility.GetUISpriteAsset(fileName), new LoadAssetCallbacks(
                (assetName, asset, duration, userData) =>
                {
        ...
        /// <param name="seasonPath"></param>
        /// <param name="path"></param>
        /// <param name="sprite"></param>
        public static void LoadLessonUISprite(string fileName, string seasonPath, string path, Image image)
        {
            GameEntry.Resource.LoadAsset(AssetUtility.GetLessonUISpriteAsset(seasonPath, path, fileName), new LoadAssetCallbacks(
                (assetName, asset, duration, userData) =>
                {
                 
                    Texture2D t2d = asset as Texture2D;
                    Rect rect = new Rect(0, 0, t2d.width, t2d.height);
                    image.sprite = Sprite.Create(t2d, rect, new Vector2(0.5f, 0.5f));
        ...
        public static void LoadGameSence(string seasonPath, string path, string fileName)
        {
        ...
        public static void LoadGameVideo(string seasonPath, string path, string fileName)
        {
            GameEntry.Resource.LoadAsset(AssetUtility.GetGameVideoAsset(seasonPath, path, fileName), new LoadAssetCallbacks(
                (assetName, asset, duration, userData) =>
                {
                 
                    Log.Info("Load GameSence Video '{0}' OK.", assetName);
                },

                (assetName, status, errorMessage, userData) =>
                {
                    Log.Error("Can not load GameSence Video '{0}' error message '{1}'.", assetName, errorMessage);         
                }));
        }


        public static void LoadGameVideo(string seasonPath, string path, string fileName, GameFrameworkAction<VideoClip> action)
        {
            GameEntry.Resource.LoadAsset(AssetUtility.GetGameVideoAsset(seasonPath, path, fileName), new LoadAssetCallbacks(
                (assetName, asset, duration, userData) =>
                {
