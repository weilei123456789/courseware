...
namespace Penny
{
    public class ProcedureLaunch : ProcedureBase
    {
        ...
        protected override void OnEnter(ProcedureOwner procedureOwner)
        {
            ...
            // 加载默认资源界面，会随 App 一起发布，故不可更新。
            GameEntry.BuiltinData.InitResourceUIForm();
        }

        private void InitializationWithConfig(Config config)
        {
            KinectManager.Instance.enabled = false;
            GameEntry.Windows.InitWindowSetting(config.Screen_Wall_Width, config.Screen_Wall_Height, config.Screen_Land_Width, config.Screen_Land_Height);
            GlobalData.Url = config.Http_IP;
            NextProduce();
        }
