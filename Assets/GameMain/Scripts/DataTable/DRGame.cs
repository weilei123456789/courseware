...
namespace Penny
{
    ...
    /// 游戏配置表。
    /// </summary>
    public class DRGame : DataRowBase
    {
        private int m_Id = 0;
        ...
        public override bool ParseDataRow(GameFrameworkSegment<string> dataRowSegment)
        {
            ...
            GameType = int.Parse(columnTexts[index++]);
            NeedLoadUI = bool.Parse(columnTexts[index++]);
            UIFormId = int.Parse(columnTexts[index++]);
        ...
        public override bool ParseDataRow(GameFrameworkSegment<byte[]> dataRowSegment)
        {
            ...
            using (MemoryStream memoryStream = new MemoryStream(dataRowSegment.Source, dataRowSegment.Offset, dataRowSegment.Length, false))
            {
                using (BinaryReader binaryReader = new BinaryReader(memoryStream, Encoding.UTF8))
                {
                    ...
                    GameType = binaryReader.ReadInt32();
                    NeedLoadUI = binaryReader.ReadBoolean();
                    UIFormId = binaryReader.ReadInt32();
                }
            }
